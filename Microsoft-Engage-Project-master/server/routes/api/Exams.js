 ...
 * Then we try to check if the same exam code is present. If not, then
 * we create the exam; else we display an error.
 */
...
/**
 * Get requests on /examByProf with exam_code and prof_email as query parameters
 * return exam object if the exam code is correct and it was created by the professor
 * with prof_email id; else returns an error.
 */
router.get("/examsByProf", (req, res) => {
    ...
    Exam.findOne({ prof_email: req_prof_email, exam_code: req_exam_code }).then(doc => {
        if (!doc) {
            return res.status(400).json("Exam doesn't exist or professor doesn't have permission");
        }
